...
class MetaFairClassifier(Transformer):
    ...
    def __init__(self, tau=0.8, sensitive_attr="", type="sr", seed=None):
        """
        Args:
    ...
    def fit(self, dataset):
        ...
        if isinstance(self.sensitive_attr, list):
            sens_idx = [dataset.protected_attribute_names.index(attribute) for attribute in self.sensitive_attr]
        else:
        ...
        y_train = np.where(dataset.labels.flatten() == dataset.favorable_label,
                           1, -1)
        if isinstance(self.sensitive_attr, list):
            print("-->dataset.privileged_protected_attributes", dataset.privileged_protected_attributes)
            x_control_train1 = np.where(
                np.isin(dataset.protected_attributes[:, sens_idx[0]],
                        dataset.privileged_protected_attributes[sens_idx[0]]),
                1, 0)
            x_control_train2 = np.where(
                np.isin(dataset.protected_attributes[:, sens_idx[1]],
                        dataset.privileged_protected_attributes[sens_idx[1]]),
                1, 0)
            # print("-->x_control_train1", x_control_train1)
            # print("-->x_control_train2", x_control_train2)
            x_control_train = np.append([[i] for i in x_control_train1], [[i] for i in x_control_train2], axis=1)
        else:
            x_control_train = np.where(
                np.isin(dataset.protected_attributes[:, sens_idx],
                        dataset.privileged_protected_attributes[sens_idx]),
                1, 0)
        print("-->x_control_train", x_control_train)
