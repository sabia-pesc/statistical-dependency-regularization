...
class StatisticalRate(General):
    ...
    def gamma(self, y_true, y_pred, sens):
        try:
            ...
            pos_0 = np.mean(y_pred[sens[:,0] == 0] == 1)
            pos_1 = np.mean(y_pred[sens[:,0] == 1] == 1)
            if pos_0 == 0 or pos_1 == 0:
                return 0
            gamma1 = min(pos_0 / pos_1, pos_1 / pos_0)
            pos_0 = np.mean(y_pred[sens[:,1] == 0] == 1)
            pos_1 = np.mean(y_pred[sens[:,1] == 1] == 1)
            if pos_0 == 0 or pos_1 == 0:
                return 0
            gamma2 = min(pos_0 / pos_1, pos_1 / pos_0)
            return (gamma1 + gamma2) / float(2)
