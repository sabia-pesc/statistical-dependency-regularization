...
class PrejudiceRemover(Transformer):
    ...
    def _create_file_in_kamishima_format(self, df, class_attr,
        ...
        if isinstance(single_sensitive.values[0], np.ndarray):
            single_sensitive_1 = single_sensitive.values[:, 0]
            single_sensitive_2 = single_sensitive.values[:, 1]
            x_append = np.array([single_sensitive_1[i] and single_sensitive_2[i] for i in range(0, len(single_sensitive_1))], dtype=np.float64)
            x.append(x_append)
        else:
    ...
    def fit(self, dataset):
        ...
        if isinstance(self.sensitive_attr, list):
            self.sensitive_ind = [all_sensitive_attributes.index(attr) for attr in self.sensitive_attr]
            sens_df_1 = pd.Series(dataset.protected_attributes[:, self.sensitive_ind[0]],
                            name=self.sensitive_attr[0])
            sens_df_2 = pd.Series(dataset.protected_attributes[:, self.sensitive_ind[1]],
                                  name=self.sensitive_attr[1])
            sens_df = pd.concat([sens_df_1, sens_df_2], axis=1)
            # sens_df = pd.Series([dataset.protected_attributes[:, self.sensitive_ind[0]],
            #                      dataset.protected_attributes[:, self.sensitive_ind[1]]],
            #                     name=[str(self.sensitive_attr[0]), str(self.sensitive_attr[1])])
        ...
        subprocess.call([sys.executable, train_pr,
                         '-e', str(self.eta),
                         '-i', train_name,
                         '-o', model_name,
                         '--quiet'])
    ...
    def predict(self, dataset):
        ...
        if isinstance(self.sensitive_attr, list):
            self.sensitive_ind = [dataset.protected_attribute_names.index(attr) for attr in self.sensitive_attr]
            if len(dataset.protected_attributes) != len(dataset.features):
                sensitive_indexs = [dataset.feature_names.index(attr) for attr in self.sensitive_attr]
                dataset.protected_attributes = np.append(np.array([np.array(dataset.features)[:, sensitive_indexs[0]]]),
                                                          np.array([np.array(dataset.features)[:, sensitive_indexs[1]]]), axis=1)
            ...
            self.sensitive_ind = dataset.protected_attribute_names.index(self.sensitive_attr)
            sens_df = pd.Series(dataset.protected_attributes[:, self.sensitive_ind],
                                name=self.sensitive_attr)
        ...
        # predict_lr RELATIVE to this file, and (b) compute & use absolute path,
        # and (c) replace python3 with python
        subprocess.call([sys.executable, predict_lr,
                         '-i', test_name,
                         '-m', self.model_name,
                         '-o', output_name,
                         '--quiet'])
