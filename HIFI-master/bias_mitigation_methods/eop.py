...
- https://github.com/chenzhenpeng18/ICSE24-Multi-Attribute-Fairness/blob/main/Fair3602/eop.py

EOP achieves Equalized Odds via a simple and efficient post-processing step,
which uses linear programming to calculate probabilities for adjusting output labels.
"""
...
def train_with_eop(dataset_name, classifier_name, seed_range=[0]):
    method_name = "eop"
    ...
    for seed in seed_range:
        ...
        eqo = EqOddsPostprocessing(privileged_groups=privileged_groups[dataset_name],
        ...
        round_result = measure_final_score(dataset_orig_test, pred_eqo, sensitive_attributes)
    ...
    with multiprocessing.Pool(processes=n_process) as pool:
        for dataset in datasets:
            for clf in classifiers:
                for seed in seed_range:
                    pool.apply_async(train_with_eop, args=(dataset, clf, [seed]))
