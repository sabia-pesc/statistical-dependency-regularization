...
Train the original models without bias mitigation methods.
"""
...
def train_original_model(dataset_name, classifier_name, seed_range=[0]):
    method_name = "default"
    ...
    with multiprocessing.Pool(processes=n_process) as pool:
        for dataset in datasets:
            for clf in classifiers:
                for seed in seed_range:
                    pool.apply_async(train_original_model, args=(dataset, clf, [seed]))
