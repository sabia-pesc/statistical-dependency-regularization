...
CEO is a post-processing technique that optimizes over calibrated classifier score outputs to find
...
def train_with_ceo(dataset_name, classifier_name, seed_range=[0]):
    method_name = "ceo"
    ...
    for seed in seed_range:
        ...
        cpp = CalibratedEqOddsPostprocessing(privileged_groups=privileged_groups[dataset_name],
    ...
    with multiprocessing.Pool(processes=n_process) as pool:
        for dataset in datasets:
            for clf in classifiers:
                for seed in seed_range:
                    pool.apply_async(train_with_ceo, args=(dataset, clf, [seed]))
